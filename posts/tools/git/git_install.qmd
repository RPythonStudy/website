...
## WSL2 Ubuntu 환경에서의 설정

-   원도우환경에서의 설치와는 달리 WSL2 Ubuntu 환경에서는 Git 설치과정에서 사실 상 자동으로 진행되므로 옵션 설정을 별도 하는 것이 필요합니다.

### 설정상태 확인

-   아래의 명령으로 전역+로컬의 설정이 저장된 위치와 함께 전체설정을 확인할 수 있습니다.

```{r git_config_check, eval=FALSE, filename="bash"}
...
-   기본 병합: Merge
    ...
    -   병합할 경우 merge commit을 생성합니다.
    -   그러나 원격저장소가 로컬의 조상인 경우에는 커밋을 생성하지는 않습니다.

-   **재배치: `rebase`**

    -   `git pull` 시, 원격 저장소의 변경사항을 먼저 가져온 후, 로컬 커밋을 그 위에 재배치합니다.
    -   커밋 히스토리가 깔끔해지지만, 충돌 발생 시 해결이 번거로울 수 있습니다.

-   **Fast-forward만 허용: `ff-only`**

    -   fast-forward 병합만 허용하며, merge commit이나 rebase를 하지 않습니다.
    -   병합이 가능한 경우에만 pull이 허용되므로, 히스토리 관리가 더욱 명확해집니다.

-   저자는 git를 시작하는 단계의 연구자들에게는 일단 Fast-forward만 허용하는 것을 추천합니다.

    -   **fast-forward 병합**은 \*\*로컬 브랜치가 원격 브랜치의 과거(commit 조상)\*\*인 경우, \*\*브랜치 포인터만 앞으로 이동(fast-forward)\*\*시키는 방식입니다. 즉, **새로운 병합 커밋을 만들지 않고도** 병합이 완료됩니다.
    -   초보자의 경우 예상치 못한 병합 커밋이나 복잡한 rebase 과정보다, **단순하고 명확한 히스토리 유지가 중요**하기 때문입니다.

-   병합과 관련된 rebasae 상태를 확인하는 명령은 아래와 같습니다.

```{r git_merge_config, eval=FALSE, filename="bash"}
git config --get pull.rebase
```

```         
-   아무런 메세지출력이 없으면 기본값인 `false`이며, 이는 `merge`를 사용한다는 것을 의미합니다.
```

-   병합과 관련된 fast-forward 상태를 확인하는 명령은 아래와 같습니다.

```{r git_ffonly_config, eval=FALSE, filename="bash"}
git config --get pull.ff
```

```         
-   아무런 메세지출력이 없으면 기본값인 `true`이며, 이는 `ff-only`를 사용한다는 것을 의미합니다.
```

-   병합과 관련된 fast-forward 상태를 변경하는 명령은 아래와 같습니다.

```{r git_ffonly_change, eval=FALSE, filename="bash"}
git config --global pull.ff only
```

-   fast-forward only로 설정된 것을 확인하는 명령은 위에서 언급되었으므로 이를 이용하여 확인하시면 됩니다.
